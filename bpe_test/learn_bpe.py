...
if __name__ == '__main__':
    ...
    vocab_map = copy.deepcopy(sorted_vocab)
    ...
    for i in range(args.symbols):
        ...
        gather[''.join(most_frequent)] = stats[most_frequent]
...
for item in sorted_vocab:
    word = ''.join(item[0]).encode('utf-8')
    word = unicode(word, 'ascii', errors='replace')
    bpe_dict[word] = item[0]

with open("dict.bpe.de",'wb') as outfile:
    a = json.dumps(bpe_dict)
    outfile.write(a)

# print gather
# with open("pairstats.bpe",'wb') as outfile:
#     a = json.dumps(gather)
#     outfile.write(a)
